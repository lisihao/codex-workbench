...
The fixed `dsh-task-template-integration-v2` profile separates a task's source-file scope from the sandbox permissions needed to write translation records and Agent Notes. Scope permission alone does not make protected metadata writable. Neither operation queues a worker, accepts a result, or transfers recovery ownership.
...
The fixed write additions are exactly twelve files: the Cordis and documentation-graph generators, the Cordis API catalog, the generated client slot catalog, the generated session event inventory, the ACP scenario registration and its two composition files, the task-template RPC package manifest, and the exact English/Chinese/pairing files for the package-wide prompt index. The documentation-graph permission is the exact `scripts/gen-doc-graphs.ts` file, never the `scripts/` directory; the prompt index permission is exactly `packages/prompt/README.md`, `README.zh.md`, and `README.i18n.yaml`, never the whole prompt directory. Those index files belong to D's integration responsibility because no accepted package owner owns the package-wide catalog. Other README and translation sidecars are not pre-authorized without a concrete changed-file or pairing failure; existing broader D scopes continue to cover documentation already owned by the task. Read-only inputs are separate exact files for the two other generators, affected template/UI/event declarations, and the existing ACP suite/launcher/config. The profile grants no Typert source directory and no translation Git helper. Task-level scope includes only uncovered exact read paths because the planner validates node read and write scopes against the task scope. E receives the same read coverage, not write access. Existing scopes remain intact.

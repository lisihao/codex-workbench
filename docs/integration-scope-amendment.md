...
The fixed additions cover the catalog generator mapping, generated runtime companions, template manifest and README records, and the registered ACP snapshot composition. Read-only generator inputs are added separately from D's writable outputs. Task-level scope must include these read paths because the planner validates both node read and write scopes against the task scope. E receives read coverage, not write access. Existing scopes remain intact.

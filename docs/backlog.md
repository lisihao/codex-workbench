...
- 在 Mac mini 上持续积累真实、非 fixture 的 Codex Radar snapshot 与 Workbench runtime 校准 Evidence，核对 SQLite `radar.sqlite3` 的 LKG、row counts、来源归因、精确 model/effort、fresh/stale/expired 和 performance provenance。个人自用 receipt 仅表示本地操作者承担责任的公共 JSON 使用决定，不是站方授权，也不声称当前已经完成生产首采。

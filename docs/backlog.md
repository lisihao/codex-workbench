...
1.9.0 已恢复手机接入：Codex 原生 Remote 管理入口、响应式手机 cockpit、认证控制和 A2 `client.observed` 回执都在当前实现与验收范围内。真实手机配对、查看和发任务仍是 external-pending Evidence，不属于 backlog。

唯一继续后置的是页面关闭后的 Web Push：需要单独决定推送提供方、设备 token 生命周期、权限提示与撤销策略。在此之前，页面打开期间的浏览器通知可以使用，但不能冒充后台 Push。

## v1.9.0 生产证据积累（evidence backlog）
...
- 取得 Codex Radar 对衍生调度集成的明确数据授权后，在 Mac mini 写入不含秘密的 authorization receipt，采集首个真实 snapshot，并核对来源归因、精确 model/effort、fresh/stale/expired 和 performance provenance。未授权期间保持 `unauthorized` 与内置 baseline 回退，不用测试夹具伪造生产数据。
- DSH 如需复用，另开独立任务只接入 `codex_radar_provider` schema/CLI 并固定 snapshot ID/digest；不得复制 Workbench SQLite、路由器或把 Radar 写成 DSH 本机 Evidence。本次版本未修改 DSH。

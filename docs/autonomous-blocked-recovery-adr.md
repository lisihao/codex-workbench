...
The `repair_source` adapter reuses `Coordinator._prepare_failed_attempt_recovery` and `DirtyWorktreeRecovery.prepare_for_retry` to restore source and dependency inputs before entering the normal executor path. Its explicit `blocked_source_repair` mode retains the original blocked result and rolls preparation failure back to blocked. It does not replace strict `source_only_recovery`, silently convert a blocked snapshot into a failed result, or drop the final verifier. Its policy and receipt integration use the existing Coordinator pool.

That transition must keep the original blocked result separate from the newly observed source delta, preserve `blocked` rather than fabricate `failed` if preparation rolls back, and retain the original allocation after both success and failure. The existing task's executor, scopes, subscription/quota gates and final verifier remain authoritative. A policy-scoped repair-dispatch budget must span successive attempts so that creating a new attempt cannot reset the permitted repair count.
...
Released 1.17.3 fixes source-retention amendment eligibility; it does not activate this autonomous mechanism. The candidate's policies remain disabled by default. Local fixtures cover structured preparation failure provenance, bounded rollback observation, linked-parent event wakeups, history-independent summaries and blocked-source repair before final acceptance. Isolated Store/Authority/Coordinator fixtures cover classification, unique repair dispatch, repaired source propagation to a successor and final verifier, original-task acceptance and preserved accepted ancestors. Session notification metadata and acknowledgements are durable and routed, but a daemon-to-Codex conversation push adapter remains unverified. Fixture results do not establish runtime deployment, activation or host delivery.

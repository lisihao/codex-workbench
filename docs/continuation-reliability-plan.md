...
Offline pnpm preparation keeps a bounded cache publication phase. If an install completed but its cache publication timed out, a later preparation of that same complete linker retries publication without reinstalling packages. Package-local linker discovery follows the lockfile's importer list; unrelated nested package-manager trees are not copied into a pnpm template. Large workspaces may use the task's declared timeout up to one hour; exhaustion remains an explicit environment blocker rather than an unbounded retry.

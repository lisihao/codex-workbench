...
MacBook bridge、MCP adapter 与 Authority 对 `workbench_control_task`、`workbench_validate_blocked_node`、`workbench_amend_task_acceptance` 的布尔值 `dry_run=true` 使用只读传输。预检参数报错后可以修正参数并沿用请求 ID；预检不创建写请求记录。旧 bridge 留下的 `sent` 记录保持不变，但不阻止只读预检；同 ID 的真实写操作仍只能查询原回执，不能借预检重放。Authority 没有预检写回执而返回 404，不代表该预检从未到达服务端，也不是重试真实写操作的许可。
...
- 当前 lease 的 settlement 若因结果校验或恢复凭据冲突被拒绝，协调器会保存被拒绝的结果工件，并把可能已经执行的节点持久化为 `indeterminate`；只有已被新 epoch、attempt 或 lease 取代的晚到结果才静默丢弃，因此线程 Future 正常结束不会留下无人持有的 `running` 节点。`accepted-source-repair` 在 target assignment 和模型启动之前失败时不制造未知副作用审批：原 accepted allocation 和结果继续作为固定输入，节点以递增 attempt 在合同 `retry_limit` 内自动续行，重启也复用同一路径；每次未执行的准备重试会把已经投递的 attempt guidance 复制到唯一下一 attempt。额度耗尽时 owner 保持带原 repair binding 的显式 `blocked` attempt，不会回退 attempt、伪装成修复完成或让依赖节点越过它。
- blocked consumer 等待的 accepted owner 若耗尽准备重试，等待投影会绑定实际失败事件并转为 `accepted_owner_repair_preparation_exhausted`。已配置且仍在任务时间/次数预算内的 `request_repair` 可进入现有维修生命周期；未配置时明确报告 `accepted_owner_repair_repair_action_unconfigured`，预算耗尽时报告 `accepted_owner_repair_repair_budget_exhausted`，责任仍归 Authority 且不会被通用计时器改写成用户责任。维修的唯一新部署凭证可续期一次有界阶段；相同凭证即使换 request id 或刷新 readiness 也不能重复消费。部署与 fresh-readiness 回执都核实后，显式授权的 `resume_owner_repairs` 只把仍未完成的 owner 以递增 attempt 重新排队，保留 blocked consumer、已验收祖先和原差量；owner 可因有界准备重试在首次目标之后的 attempt 验收，后续 consumer 恢复要求其 attempt 不低于原目标、状态为 accepted 且无残留 recovery。已经派单或关联的维修只根据其实际等待、部署和 fresh-readiness 回执推进，不以定时轮询冒充完成。paused/cancelled 状态优先，部署回执不会续期预算或启动该动作。
- 若维修在 Workbench 任务图之外完成，认证客户端可调用 `workbench_resume_owner_repairs` 接纳该部署。调用必须精确声明当前安装的 version、commit 和 tag，匹配当前任务 revision、blocked consumer attempt 及已启用的 `resume_owner_repairs` 策略。Authority 还会核对安装时间晚于每个耗尽事件、当前协调器在该安装之后启动，并从安装清单哈希和 authority epoch 生成内容寻址的 repair/readiness 指纹；任何陈旧身份、重复消费或 CAS 漂移均拒绝。该入口只续行耗尽 owner，不发布、不部署，也不接受原任务。

...
基线：用户提供的 578 行《Codex 工作台完整设计方案》及后续范围变更。本矩阵描述 1.7.0 代码候选，不把接口、fixture 或一次进程启动等同于真实端到端完成。
...
1. 控制面、执行面、配额治理、持久状态、工作树隔离和证据验收已按原设计实现；1.7.0 另加入性能基线、长期账本和 Spark P0 调度闭环。
2. 1.7.0 的 routing-v3 以版本化能力目录和 pinned performance snapshot 作为新任务路由输入：硬门禁先于保守质量下界、角色适配、成本和时延排序；Sol 不做常规 Worker，Claude 继续受认证和 20% 保留政策约束。
...
4. 1.7.0 在完成当前 A1–A12 外部旅程前仍应称为“设计兼容实现”，不能称为“全部验收完成”；尤其不应把 quota/capability sidecar 的夹具验证描述为登录态生产验证或真实模型质量基准。

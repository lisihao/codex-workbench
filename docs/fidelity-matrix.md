...
基线：用户提供的 578 行《Codex 工作台完整设计方案》及后续范围变更。本矩阵描述 1.13.0 代码候选，不把接口、fixture 或一次进程启动等同于真实端到端完成。
...
| Claude 20% 目标保留、25% 停线 | compatible-subset | 五小时、周全模型与 CLI 当前显示的一个模型专属池；已显示的 Sonnet/Fable 专属池叠加于全模型池，未显示的模型使用全模型周池；30% admission guard、25% 硬停线、共享容量、每个 Claude 节点后要求新快照；未知即 Codex | 被动显示接口不能约束单回合消耗；绝不跨越 20% 仍需真实窗口 Evidence，不能静态保证 |
...
1. 控制面、执行面、配额治理、持久状态、工作树隔离和证据验收已按原设计实现；1.11.0 包含 Codex Radar 与 AI Frontier 两个通用离线 Provider、多来源弱先验、SQLite 权威数据存储、性能基线、长期账本、Spark P0 调度闭环、可恢复 NAS 归档生命周期，以及受管 Sol/Terra/Luna 的 500K 显式长上下文覆盖。
2. 1.11.0 的 routing-v3 以版本化能力目录和 pinned performance snapshot 作为新任务路由输入：硬门禁后先保守质量和验收风险等价带，再以用户/角色偏好、预计尝试、返工、时延、一致性风险、相对成本、吞吐和利用率确定性排序；Sol 不做常规 Worker，Claude 继续受认证和 20% 保留政策约束。
3. 公开 benchmark、Radar 与 AI Frontier 只提供按来源、领域和 exact identity 带迁移折扣的冷启动弱先验，不是统一排行榜，也不是本机成功率；来源未 consent/过期时回落内置 baseline，当前 performance calibrate 只报告 advisory `cold-start` 或 `ok`，尚未实现 promotion 阈值，不能称为动态校准完成。
4. 1.11.0 在完成当前 A1–A12 外部旅程前仍应称为“设计兼容实现”，不能称为“全部验收完成”；尤其不应把 quota/capability sidecar 的夹具验证描述为登录态生产验证或真实模型质量基准。

...
`workbench_request` validates its request fields and routing strategy before it can enqueue a planning reservation. A known invalid field returns a durable `rejected` receipt with `effects: none`, `enqueued: false`, and bounded allowed/invalid field lists. The Codex MCP adapter preserves that receipt as an `isError: true` tool result, while `service invoke` prints the same terminal receipt instead of reporting a transport failure. Repeating the same request ID returns that rejection. Exceptions after the owned operation begins still settle as `unknown`; a `ValueError` or `TypeError` alone is never evidence that execution had no effect.
...
The diagnostic separates configured transport, SSH/local child, Authority HTTP, and MCP child. Cockpit `/api/snapshot` returns at most the ten most recently updated complete task cards plus explicit total/truncation metadata; its stale-task diagnostic reads only task identifiers, states and timestamps, and older complete cards remain available through the paginated task and event endpoints. An Authority service request that remains in one stage for two seconds writes a bounded local code-location trace without request data. A snapshot timeout is not sufficient evidence of a dead Authority, so connection health uses the lightweight service status endpoint. Workbench MCP and Codex's separate remote AppServer channel are different connections.

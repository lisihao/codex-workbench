...
The diagnostic separates configured transport, SSH/local child, Authority HTTP, and MCP child. Cockpit `/api/snapshot` returns at most the ten most recently updated complete task cards plus explicit total/truncation metadata; its stale-task diagnostic reads only task identifiers, states and timestamps, and older complete cards remain available through the paginated task and event endpoints. A snapshot timeout is not sufficient evidence of a dead Authority, so connection health uses the lightweight service status endpoint. Workbench MCP and Codex's separate remote AppServer channel are different connections.

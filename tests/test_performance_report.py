...
class PerformanceReportTests(unittest.TestCase):
    ...
    def test_csv_and_html_have_explicit_units_dates_and_escaped_text(self) -> None:
        ...
        self.assertIn("<title>分来源性能证据账本（非统一排行榜）", html)
        self.assertIn("本页按来源分账，不形成统一排行榜", html)
        self.assertIn("不同 benchmark、推理档位、harness 及成本单位不可直接横向比较", html)
        self.assertIn("行数不等于独立质量样本数", html)
        self.assertIn("N/A 代表未知或不适用，不是 0", html)
        self.assertIn("采集/型号匹配不等于实测路由收益", html)
